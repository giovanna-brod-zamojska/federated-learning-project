# taken from https://github.com/iurada/talos-task-arithmetic/blob/ae102473de0e57ebf625eca22e10781c371149ec/vision/pruners.py

import gc
import torch
import classes.model_editing.layers as layers
...
def prunable(module, batchnorm, residual):
    ...
    isprunable = isinstance(
        module,
        (layers.MultiheadAttention, layers.Linear, layers.Conv2d, layers.LayerNorm),
    )
    return isprunable
...
class Pruner:
    ...
    def _global_copy(self, sparsity):
        ...
        global_masks = torch.cat(
            [
                ...
                if id(p) in self.scores
            ]
        )
        t, _ = torch.kthvalue(
            global_scores[global_masks == 1.0],
            min(
                int(sparsity * global_scores.numel()),
                global_scores[global_masks == 1.0].numel() - 1,
            ),
        )
        print("[+] Threshold:", t)
        tot, not_masked = 0, 0
    ...
    def _random_copy(self, sparsity):
        for mask, param in self.masked_parameters:
            if id(param) in self.scores:
                ...
                dw_threshold, _ = torch.kthvalue(
                    torch.flatten(score), int(sparsity * score.numel())
                )  #!
    ...
    def _mag_copy(self, sparsity):
        global_scores = torch.cat(
            [
                torch.flatten(v).abs()
                for _, v in self.masked_parameters
                if id(v) in self.scores
            ]
        )
        dw_threshold, _ = torch.kthvalue(
    ...
    def _global_mask(self, sparsity):
        ...
        if not k < 1:
            ...
            for mask, param in self.masked_parameters:
                if id(param) not in self.scores:
                    continue
                ...
                for idx in range(data.size(0)):
                    ...
                    for m, p in masked_parameters(model):
                        if (
                            ...
                            self.scores[id(p)] += (
                                torch.clone(p.grad.data.pow(2)).detach().cpu()
                            )
