# taken from https://github.com/iurada/talos-task-arithmetic/blob/main/vision/layers.py
    ...
    def __init__(
        ...
        if not self._qkv_same_embed_dim:
            ...
            self.register_buffer(
                "q_proj_weight_mask", torch.ones(self.q_proj_weight.shape)
            )
            self.register_buffer(
                "k_proj_weight_mask", torch.ones(self.k_proj_weight.shape)
            )
            self.register_buffer(
                "v_proj_weight_mask", torch.ones(self.v_proj_weight.shape)
            )
        else:
            ...
            self.register_buffer(
                "in_proj_weight_mask", torch.ones(self.in_proj_weight.shape)
            )
        ...
        if bias:
            ...
            self.register_buffer(
                "in_proj_bias_mask", torch.ones(self.in_proj_bias.shape)
            )
        else:
    ...
    def forward(
        ...
        if not why_not_fast_path:
            ...
            if not why_not_fast_path:
                ...
                return torch._native_multi_head_attention(
                    ...
                    mm(
                        self.in_proj_weight,
                        self.in_proj_weight_mask,
                        masking=self.masking,
                    ),
                    mm(self.in_proj_bias, self.in_proj_bias_mask, masking=self.masking),
                    mm(
                        self.out_proj.weight,
                        self.out_proj.weight_mask,
                        masking=self.masking,
                    ),
                    mm(
                        self.out_proj.bias,
                        self.out_proj.bias_mask,
                        masking=self.masking,
                    ),
                    merged_mask,
                    need_weights,
        ...
        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights = F.multi_head_attention_forward(
                ...
                mm(self.in_proj_bias, self.in_proj_bias_mask, masking=self.masking),
                mm(self.bias_k, self.bias_k_mask, masking=self.masking),
                mm(self.bias_v, self.bias_v_mask, masking=self.masking),
                self.add_zero_attn,
                self.dropout,
                mm(
                    self.out_proj.weight,
                ...
                q_proj_weight=mm(
                    self.q_proj_weight, self.q_proj_weight_mask, masking=self.masking
                ),
                k_proj_weight=mm(
                    self.k_proj_weight, self.k_proj_weight_mask, masking=self.masking
                ),
                v_proj_weight=mm(
                    self.v_proj_weight, self.v_proj_weight_mask, masking=self.masking
                ),
...
def mask_pretrained_vit(model, device, dtype, skip_ln=False):
    ...
    for name, layer in model.named_modules():
        ...
        if isinstance(layer, nn.Conv2d):
            ...
            name = re.sub(r"(\.)(\d+)(\.)", r"[\2].", name)
            exec(
                f"model.{name} = MultiheadAttention( \
